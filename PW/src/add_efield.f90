...
SUBROUTINE add_efield(vpoten,etotefield,rho,iflag)
  ...
  USE extfield,      ONLY : tefield, dipfield, edir, eamp, emaxpos, &
                            eopreg, estep1, estep2, estart, eend, forcefield
  ...
  IF (ionode) THEN
       ...
       !
       WRITE( stdout,*)
       WRITE( stdout,'(5x,"Adding bias electric potential":)')
       ...
       IF (abs(eamp)>0._dp) WRITE( stdout, &
          '(8x,"Bias between left and right electrode is [Ry a.u.]: ", es11.4)') eamp 
        
       WRITE( stdout,'(8x,"Left electrode starts at", f11.4," bohr, ends at", f11.4," bohr")') length*estart, length*emaxpos
       WRITE( stdout,'(8x,"Left screen zone starts at", f11.4," bohr, ends at", f11.4," bohr")') length*emaxpos, length*(emaxpos+estep1)
       WRITE( stdout,'(8x,"Right screen zone starts at", f11.4," bohr, ends at", f11.4," bohr")') length*(emaxpos+eopreg-estep2), length*(emaxpos+eopreg)
       WRITE( stdout,'(8x,"Right electrode starts at", f11.4," bohr, ends at", f11.4," bohr")') length*(emaxpos+eopreg), length*eend
       WRITE( stdout,*)     
  ENDIF
  ...
  !  Add potential
  !
  !  V\left(ijk\right) = eamp/2 * frac{k}{nr3}/estart				0			<= frac{k}{nr3} < estart
  !                      eamp/2							estart			<= frac{k}{nr3} < emaxpos
  !			 eamp/2 * (1 - (frac{k}{nr3}-emaxpos)/estep1)		emaxpos			<= frac{k}{nr3} < emaxpos+estep1
  !			 0							emaxpos+estep1		<= frac{k}{nr3} < emaxpos+eopreg-estep2
  !			-eamp/2 * (frac{k}{nr3}-emaxpos-eopreg+estep2)/estep2	emaxpos+eopreg-estep2	<= frac{k}{nr3} < emaxpos+eopreg
  !			-eamp/2							emaxpos+eopreg		<= frac{k}{nr3} < eend
  !			-eamp/2 * (1 - (frac{k}{nr3}-eend)/(1-eend))		eend			<= frac{k}{nr3} < 1
  !---------------------
  ...
  DO ir = 1, dfftp%nnr
     ...
     if ( sawarg < estart ) value = 0.5d0 * eamp * (sawarg/estart)
     if ( sawarg>=estart .and. sawarg<emaxpos ) value = 0.5d0 * eamp
     if ( sawarg>=emaxpos .and. sawarg<(emaxpos+estep1) ) value = 0.5d0 * eamp * (1.0d0 - (sawarg-emaxpos)/estep1)
     if ( sawarg>=(emaxpos+estep1) .and. sawarg<(emaxpos+eopreg-estep2) ) value = 0.0d0
     if ( sawarg>=(emaxpos+eopreg-estep2) .and. sawarg<(emaxpos+eopreg) ) value = -0.5d0 * eamp * (sawarg-emaxpos-eopreg+estep2)/estep2
     if ( sawarg>=(emaxpos+eopreg) .and. sawarg<eend) value = -0.5d0 * eamp
     if ( sawarg>=eend) value = -0.5d0 * eamp * (1 - (sawarg-eend)/(1.0d0-eend))
